                        ...
                        <el-row>
                            ...
                            <el-col :span="11" :lg="12" :md="20" :sm="20" :xs="20">
                                ...
                                <span >大图用于大图预览以及详情页展示</span>
                                   <el-dialog title="个人头像" :visible.sync="isImgUrlVisible1" width="30%" @close="isImgUrlVisible1=false">
                                     <img :src='imgUrl1' width="100%"/>
                                    </el-dialog>
                            </el-col>
                            <!-- <el-col :span="14">
                                <el-form-item>
    ...
    httpRequestSettings1(param) {
      ...
      xhr.onreadystatechange = function() {
        if (xhr.readyState == 4) {
          if ((xhr.status >= 200 && xhr.status < 300) || xhr.status == 304) {
            that.$message.success("上传图片成功");
            // that.imgUrl1 = `${that.siteUrl}${
            //   JSON.parse(xhr.responseText).path
            // }`;
            // that.form.serverUrl = `${that.siteUrl}${
            //   JSON.parse(xhr.responseText).path
            // }`;
