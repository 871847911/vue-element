          ...
          <el-row>
            ...
            <el-col :span="7">
              ...
              <el-form-item label="" prop="authorImgUrl" style="margin-top:8px">
                <el-upload class="avatar-uploader" :action="siteUrl+'/upload'" :limit=1 list-type="picture-card" :http-request="httpRequestSettings" :on-preview="handlePreview" :before-upload="beforeAvatarUpload" :on-remove="handleRemove" :file-list="fileList">
        ...
        <el-row>
          ...
          <el-col :span="7">
            <el-form-item label="" prop="introduction">
              <el-input v-model="form.introduction" type="textarea" :autosize="{ minRows: 4, maxRows: 6}" maxlength='400'></el-input>
...
export default {
  ...
  computed: {
    ...
    remain: function() {
      return 400 - this.form.introduction.length;
    }
  },
  methods: {
    showUp(item) {
    ...
    getTechTypeList() {
      ...
      api.selectByStore(params).then(res => {
        ...
        if (res.length == 0) {
          ...
          this.typeOfSerList = res.reverse();
          this.form.type = this.typeOfSerList[0].name;
          // this.form.type = res.reverse()[0]
        }
      });
    },
      ...
      xhr.onreadystatechange = function() {
        if (xhr.readyState == 4) {
          if ((xhr.status >= 200 && xhr.status < 300) || xhr.status == 304) {
            ...
            // that.form.authorImgUrl = `${that.siteUrl}${
            //   JSON.parse(xhr.responseText).path
            // }`;
            that.imgUrl = JSON.parse(xhr.responseText).url;
            that.form.authorImgUrl = JSON.parse(xhr.responseText).url;
            that.isShow = false;
            // that.fileList.push({name:'',url:this.imgUrl})
          } else {
        ...
        if (valid) {
          ...
          if (typeof this.form.type == "object") {
            ...
            params = {
              ...
              headUrl: this.form.authorImgUrl,
              detail: this.form.introduction,
              staffTypeId: this.form.type.id,
              staffTypeName: typeName,
              sellerId: this.loginInfo.sellerId,
                createDate:new Date(new Date()).toLocaleDateString().replace(/\//g, "-")
            };
          } else if (typeof this.form.type == "string") {
            typeName = this.form.type;
            typeId = this.typeOfSerList.find(
              item => item.name == this.form.type
            ).id;
            params = {
              // staff: {
              isEnable: 1,
