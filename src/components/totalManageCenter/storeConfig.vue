        ...
        <div class="configInfo">
            <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="150px" class="demo-ruleForm">
                ...
                <el-form-item label="门店图片：" prop="storeImg">
                    <p>推荐图片尺寸400*400px，大小不超过1MB</p>
                    <el-upload
                        class="avatar-uploader"
                        :action="siteUrl+'/upload'"
...
<script>
    ...
    export default {
        ...
        methods: {
            //获取门店配置信息
            getQueryStorePage() {
                let params = { page: { }, store: { id: this.$route.query.id} };
                api.queryStorePage(params).then(res => {
                    console.log(res.rows[0])
                    let configInfo = res.rows[0]
                    console.log(configInfo.regionName.split('-').map(Number))
                    this.ruleForm.storeName = res.rows[0].name;
                    this.ruleForm.storeOwner = res.rows[0].linkman;
                    this.ruleForm.tel = res.rows[0].phone;
                    this.ruleForm.phone1 = res.rows[0].zoneNum;
                    this.ruleForm.phone2 = res.rows[0].zphone;
                    this.ruleForm.storeArea = configInfo.regionName.split('-').map(Number)
                    this.ruleForm.marked = res.rows[0].coordinateAdress
                    this.ruleForm.storeAdd = res.rows[0].address;
                    this.ruleForm.storeImg = res.rows[0].picUrl;
                    this.ruleForm.startTime = res.rows[0].openingTime;
                    this.ruleForm.endTime = res.rows[0].closingTime;
                    this.lat = res.rows[0].addressY;
                    this.lng = res.rows[0].addressX
                });
            },
            // 更新门店配置
            updateStore() {
                ...
                xhr.onreadystatechange = function () {
                    if (xhr.readyState == 4) {
                        if ((xhr.status >= 200 && xhr.status < 300) || xhr.status == 304) {
                            ...
                            console.log(xhr.previewText)
                            // that.ruleForm.storeImg = `${that.siteUrl}${JSON.parse(xhr.responseText).path}`;
